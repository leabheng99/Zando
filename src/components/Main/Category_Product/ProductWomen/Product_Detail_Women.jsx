...
const Product_Detail_Women = () => {
  const [hoverId, setHoverId] = useState(null);
  const { id } = useParams();
  const { addToCart } = useContext(CartContext); // ✅ Get addToCart function

  const product = products_women.find(p => p.id === parseInt(id));
      ...
      <div className="flex flex-col md:flex-row gap-3 lg:gap-10">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-start">
          ...
          <p className="text-gray-700 mb-6">
            This is a high-quality product for women. Perfect for daily wear or special occasions.
          </p>
...
export default Product_Detail_Women;
