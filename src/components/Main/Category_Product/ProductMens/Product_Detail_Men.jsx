...
import { products_men, products_women } from '../../../data';
import { FaRegHeart } from "react-icons/fa";
import { CartContext } from '../../../Context/CartContext'; // ✅ Import CartContext

const Product_Detail_Men = () => {
  const [hoverId, setHoverId] = useState(null);
  const { id } = useParams();
  const { addToCart } = useContext(CartContext); // ✅ Get addToCart function

  // Find product in men products first, then women products
  let product = products_men.find(p => p.id === parseInt(id));
  if (!product) {
    product = products_women.find(p => p.id === parseInt(id));
  }
      ...
      <div className="flex flex-col md:flex-row gap-3 lg:gap-10">
        ...
        <div className="w-full md:w-1/2">
          ...
          <p className="mb-6 text-gray-700">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Praesent at quam vel mauris 
            suscipit tincidunt. Suspendisse potenti.
          </p>
...
export default Product_Detail_Men;
